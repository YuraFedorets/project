...
Використовує render_template замість render_template_string.
"""

import uuid
import threading
from flask import request, session, redirect, flash, jsonify, render_template, g
...
def register_routes(app):
    ...
    def render(active_tab='home', **kwargs):
        ...
        if session.get('role') == 'ADMIN':
            res = db.execute(
                "SELECT COUNT(*) as c FROM support_messages WHERE is_read=0 AND sender_type != 'admin' AND is_archived=0"
            ).fetchone()
    ...
    def index():
        ...
        if active_tab == 'ranking':
            unique_courses     = [r['course'] for r in db.execute(
                "SELECT DISTINCT course FROM students WHERE course IS NOT NULL AND course != '' ORDER BY course"
            ).fetchall()]
            unique_specialties = [r['specialty'] for r in db.execute(
                "SELECT DISTINCT specialty FROM students WHERE specialty IS NOT NULL AND specialty != '' ORDER BY specialty"
            ).fetchall()]
        ...
        if active_tab in ('users', 'companies') and session.get('role') == 'ADMIN':
            all_students = [dict(r) for r in db.execute(
                "SELECT * FROM students ORDER BY last_name"
            ).fetchall()]
            for comp in db.execute("SELECT * FROM companies ORDER BY company_name").fetchall():
                c = dict(comp)
                c["employees"] = [dict(e) for e in db.execute(
                    "SELECT * FROM users WHERE company_id=? ORDER BY role,username", (c["id"],)
                ).fetchall()]
                all_companies.append(c)
        if not all_companies:
            for comp in db.execute("SELECT * FROM companies ORDER BY company_name").fetchall():
                ...
                if session.get('edit_target_id'):
                    ...
                    if row:
                        ...
                        if row:
                            user_info = dict(row)
                            comp = db.execute(
                                "SELECT * FROM companies WHERE id = ?", (user_info.get('company_id'),)
                            ).fetchone()
        ...
        if session.get('role') == 'STUDENT':
            res = db.execute(
                "SELECT COUNT(*) as c FROM invitations i WHERE i.student_id = ? AND i.status='pending'",
                (session['user_id'],)
            ).fetchone()
            pending_count = res['c']
        ...
        if session.get('role') == 'ADMIN' and active_tab == 'support':
            ...
            if active_conv_key:
                active_conv_messages = [dict(m) for m in db.execute(
                    "SELECT * FROM support_messages WHERE session_key=? ORDER BY created_at ASC",
                    (active_conv_key,)
                ).fetchall()]
                db.execute(
                    "UPDATE support_messages SET is_read=1 WHERE session_key=? AND sender_type!='admin'",
                    (active_conv_key,)
                )
                db.commit()
                sender_row = db.execute(
                    "SELECT sender_name FROM support_messages WHERE session_key=? AND sender_type!='admin' LIMIT 1",
                    (active_conv_key,)
                ).fetchone()
    ...
    def update_profile():
        ...
        if target_role == 'STUDENT':
            ...
            if new_email and role == 'ADMIN':
                db.execute("UPDATE users SET email=?, username=? WHERE id=?",
                           (new_email, new_email, target_id))
    ...
    def send_invite():
        ...
        db.execute(
            "INSERT INTO invitations (student_id, company_id, user_id, message, status) VALUES (?, ?, ?, ?, 'pending')",
    ...
    def admin_toggle_block():
        ...
        if user_type == 'student':
            db.execute(
                "UPDATE students SET status = CASE WHEN status='blocked' THEN 'active' ELSE 'blocked' END WHERE id=?",
                (user_id,)
            )
            tab = 'users'
        else:
            db.execute(
                "UPDATE users SET status = CASE WHEN status='blocked' THEN 'active' ELSE 'blocked' END WHERE id=?",
                (user_id,)
            )
            tab = 'companies'
        db.commit()
    ...
    def admin_create_company():
        ...
        try:
            ...
            db.execute(
                "INSERT INTO users (username, email, password, role, company_id, position, status) "
                "VALUES (?, ?, ?, 'COMPANY_ADMIN', ?, 'Директор', 'active')",
